...
const Question = ({ mongoUserId }: QuestionProps) => {
	...
	const handleTagRemove = (tag: string, field: any) => {
		const newTags = field.value.filter((acceptedTag: string) => acceptedTag !== tag);
			...
			>
				...
				<FormField
					...
					render={({ field }) => (
						<FormItem className='flex w-full flex-col'>
							...
							<FormDescription className='body-regular mt-2.5 text-light-500'>
								Add up to 3 tags to describe what your question is about. You need rto press enter to add a tag.
							</FormDescription>
							<FormMessage className='text-red-500' />
						</FormItem>
					)}
				...
				>
					{isSubmitting ? (
						...
						<Fragment>{type === 'edit' ? 'Edit Question' : 'Ask Question'}</Fragment>
					)}
				</Button>
			</form>
		</Form>
